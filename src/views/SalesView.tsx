...
import { ShoppingCart, Plus, Mic, Search, Filter, CheckCircle, Clock, AlertCircle, FileText, X } from 'lucide-react';
...
export const SalesView: React.FC<SalesViewProps> = ({ onOpenVoiceModal }) => {
  ...
  const handleProductSelect = (prodId: string) => {
    ...
    if (prod) {
      ...
      setPaidValue(prod.price * quantity);
    }
  };
  ...
  const handleQtyChange = (qty: number) => {
    ...
    if (prod) {
      setPaidValue((prod.price * qty) - discount);
    }
  };
  ...
  const handleSubmitNewSale = (e: React.FormEvent) => {
    ...
    const totalVal = (unitPrice * quantity) - discount;
    const pendingVal = Math.max(0, totalVal - paidValue);
    ...
    const newSale: Sale = {
      ...
      notes,
      status: pendingVal === 0 ? 'Concluída' : (paidValue > 0 ? 'Parcial' : 'Pendente')
    };
    ...
    <div className="space-y-6 pb-20">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-amber-800/80">Registre vendas rapidamente por formulário ou por voz.</p>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl border border-amber-200 space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-amber-100 pb-3">
              <h3 className="font-bold text-amber-950 text-lg">Lançar Nova Venda Manual</h3>
              <button onClick={() => setIsModalOpen(false)} className="text-amber-700 hover:text-amber-950">
                <X className="w-5 h-5" />
              </button>
            </div>

            <form onSubmit={handleSubmitNewSale} className="space-y-4 text-xs sm:text-sm">
              <div>
              ...
              <div>
                <label className="block font-bold text-amber-900 mb-1">Produto:</label>
                <select
                  required
              ...
              <div className="bg-amber-100/60 p-3 rounded-xl border border-amber-200">
                <div className="flex items-center justify-between font-bold text-amber-950">
                  ...
                  <span>R$ {((unitPrice * quantity) - discount).toFixed(2)}</span>
                </div>
              </div>

              <div>
                <label className="block font-bold text-amber-900 mb-1">Valor Pago Agora (R$):</label>
                <input
                  type="number"
                  step="0.01"
                  value={paidValue}
                ...
                />
                {((unitPrice * quantity) - discount - paidValue) > 0 && (
                  <p className="text-xs text-red-600 font-semibold mt-1">
                    Saldo pendente/fiado: R$ {((unitPrice * quantity) - discount - paidValue).toFixed(2)}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="Ex: Entregar sábado na loja"
                  value={notes}
