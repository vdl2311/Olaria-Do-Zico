    ...
    <div className="min-h-screen bg-stone-100 flex flex-col justify-between p-4 sm:p-6 selection:bg-amber-800 selection:text-white">
      ...
      <main className="max-w-md w-full mx-auto my-auto py-3 space-y-4">
        ...
        {activeTab === 'production' && (
          <div className="bg-white border border-stone-200 rounded-3xl shadow-xs p-6 sm:p-8 space-y-5">
            ...
            {authMethod === 'none' && (
              <form onSubmit={handlePasswordLogin} className="space-y-4 font-brand-sans">
                {/* Identifier */}
                <div>
                  <label htmlFor="login-identifier" className="block text-xs font-bold text-[#292724] mb-1.5">
                    E-mail ou celular
                  </label>
                  <div className="relative">
                    <Mail className="w-4 h-4 text-[#8A5A44] absolute left-3.5 top-3.5" />
                    <input
                      id="login-identifier"
                      type="text"
                      required
                      ...
                      className="w-full pl-10 pr-3.5 py-2.5 rounded-xl border border-[#D4BEA2] text-sm focus:border-[#B85C38] focus:ring-2 focus:ring-[#B85C38]/20 outline-hidden transition-all bg-white font-medium text-[#292724]"
                    />
                  </div>
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <label htmlFor="login-password" className="block text-xs font-bold text-[#292724]">
                      Senha
                    </label>
                    <button
                      type="button"
                      ...
                      className="text-xs font-semibold text-[#B85C38] hover:text-[#9E4A2A] hover:underline cursor-pointer focus-visible:outline-2 focus-visible:outline-[#B85C38] rounded-sm"
                    >
                      Esqueci minha senha
                    </button>
                  </div>
                  <div className="relative">
                    <Lock className="w-4 h-4 text-[#8A5A44] absolute left-3.5 top-3.5" />
                    <input
                      id="login-password"
                      ...
                      placeholder="••••••••"
                      className="w-full pl-10 pr-10 py-2.5 rounded-xl border border-[#D4BEA2] text-sm focus:border-[#B85C38] focus:ring-2 focus:ring-[#B85C38]/20 outline-hidden transition-all bg-white font-medium text-[#292724]"
                    />
                    <button
                      type="button"
                      onClick={() => setShowPassword(!showPassword)}
                      aria-label={showPassword ? "Ocultar senha" : "Exibir senha"}
                      className="absolute right-3 top-3 text-[#8A5A44] hover:text-[#292724] cursor-pointer focus-visible:outline-2 focus-visible:outline-[#B85C38] rounded-sm"
                    >
                ...
                <div className="flex items-center justify-between pt-0.5">
                  <label htmlFor="login-remember" className="flex items-center gap-2 text-xs text-[#5C5852] cursor-pointer select-none">
                    <input
                      id="login-remember"
                      type="checkbox"
                      checked={rememberMe}
                      onChange={(e) => setRememberMe(e.target.checked)}
                      className="w-4 h-4 rounded border-[#D4BEA2] text-[#B85C38] focus:ring-[#B85C38]"
                    />
      ...
      {showHandoverModal && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-xs flex items-end sm:items-center justify-center p-0 sm:p-4 overflow-y-auto animate-in fade-in duration-150">
          <div className="bg-white rounded-t-3xl sm:rounded-2xl max-w-lg w-full border border-amber-300 shadow-2xl flex flex-col max-h-[92dvh] sm:max-h-[88vh] overflow-hidden animate-in slide-in-from-bottom-6 sm:slide-in-from-bottom-2">
            {/* Sticky Header */}
            <div className="p-4 sm:p-5 flex items-start justify-between border-b border-amber-100 shrink-0 bg-white">
              <div className="flex items-center gap-2.5">
                <div className="w-10 h-10 rounded-2xl bg-amber-900 text-amber-100 flex items-center justify-center shadow-md shrink-0">
                  <Building2 className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="text-base sm:text-lg font-black text-amber-950">Ativação & Titularidade</h3>
                  <p className="text-[11px] sm:text-xs text-amber-800 font-medium">Entrega oficial do sistema para o proprietário</p>
                </div>
              </div>
              <button
            ...
            <form onSubmit={handleHandoverSubmit} className="flex flex-col flex-1 overflow-hidden">
              <div className="p-4 sm:p-6 overflow-y-auto space-y-4 flex-1 overscroll-contain text-xs sm:text-sm">
                <div className="bg-amber-50/90 border border-amber-200 rounded-2xl p-3.5 sm:p-4 space-y-2 text-xs text-amber-950">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 rounded-xl border border-stone-200 text-stone-600 font-mono text-xs bg-stone-50 outline-hidden"
                  />
                </div>
              </div>
              ...
              <div className="p-3.5 sm:p-4 border-t border-amber-100 flex items-center justify-end gap-2 bg-amber-50/60 shrink-0">
                ...
                >
                  ...
                  <span>{isLoading ? 'Transferindo Posse...' : 'Assumir Titularidade'}</span>
                </button>
              </div>
            </form>
          </div>
      ...
      {showResetModal && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-xs flex items-end sm:items-center justify-center p-0 sm:p-4 overflow-y-auto animate-in fade-in duration-150">
          <div className="bg-white rounded-t-3xl sm:rounded-2xl max-w-md w-full border border-stone-200 shadow-2xl flex flex-col max-h-[90dvh] sm:max-h-[85vh] overflow-hidden">
            ...
            {resetStep === 'request' ? (
              <form onSubmit={handleRequestReset} className="flex flex-col flex-1 overflow-hidden">
              ...
              <form onSubmit={handleConfirmReset} className="flex flex-col flex-1 overflow-hidden">
                <div className="p-4 sm:p-5 space-y-3 overflow-y-auto flex-1 text-xs">
                  {resetSuccessMessage && (
